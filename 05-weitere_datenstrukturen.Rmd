...
Als erstes einfaches Beispiel soll eine Liste in R erzeugt werden, die Prüfungsdaten eines Studierenden enthält:
...
`x` ist also eine Liste mit drei Elementen; jedes Element hat dabei einen Namen (Daher oft auch *named* list (oder *benannte* Liste) genannt.). Die Namen werden auch als „Tags“ bezeichnet..  Das dritte Listenelement zeigt auf einen numerischen Vektor der Länge 2. Weiterhin erhalten wir:
...
Überraschenderweise geben `x["noten"]` und `x[3]` eine Liste zurück. Der einfache Indexoperator angewandt auf eine Liste gibt immer eine Teilliste zurück! Will man also das Element einer Liste erhalten, so muss der doppelte Indexoperator benutzt werden. Aus Konsistenzgründen empfehlen daher manche Autoren sogar, auch bei Vektoren den doppelten Indexoperator zu benutzen, da es bei Vektoren keinen Unterschied zwischen beiden Schreibweisen gibt:
...
```{r, echo=FALSE, fig.cap="(ref:figlist-vektor-1)", out.width="60%"}
...
```{r, echo=FALSE, fig.cap="(ref:figlist-vektor-2)", out.width="60%"}
...
Da eine Liste ein linear angeordneter Datencontainer ist, dessen Elemente auf andere Objekte verweisen und der als ein Vektor von Referenzen aufgefasst werden kann, ergibt sich im Vergleich zum Vektor eine andere interne Implementierung. Dies wird im folgenden Diagramm deutlich:
...
Mithilfe von Faktoren ist es in R möglich, nominalskalierte oder ordinalskalierte Daten (Oft wird in diesem Zusammenhang auch von kategorialen Variablen gesprochen.) zu bearbeiten.
...
Als Beispiel wollen wir einen Datensatz `data` betrachten, der aus 30 nominalskalierten Werten besteht, die den belegten Studiengang Maschinenbau (MB), Elektrotechnik (ET), Wirtschaftsingenieur (WIng) und Wirtschaftsinformatik (WInfo) von 30 Studierenden angibt:
...
Mit der Funktion `levels()` können wir die entsprechenden Level erhalten, die bei der Umwandlung automatisch erzeugt wurden. Wenn wir wollen, dass "MB" für 1, "ET" für 2, "WIng" für 3 und "WInfo" für 4 steht, dann müssen wir die Level explizit in der entsprechenden Reihenfolge angeben: 
```{r}
...
Mithilfe der Funktion `table()` (siehe Übungen zu Kapitel \@ref(homogenedatenstrukturen)) kann ein Faktor auch in Tabellenform angezeigt werden:
```{r}
table(f1)
```

Ein ähnliches Ergebnis erhält man auch mit der generischen Funktion `summary()` (Die erzeugten Ausgaben sind fast identisch, aber die Rückgabewerte sind unterschiedlich: Während `table()` ein Objekt der Klasse `table` zurückgibt, gibt in diesem Fall `summary()` einen benannten Vektor zurück.):
```{r}
summary(f1)
```
...
Die Daten in `f1` sind nominalskaliert, jedoch nicht ordinalskaliert. Daher ist die folgende Fehlermeldung nicht überraschend:
...
Viele Datensätze liegen in Form von Tabellen vor, da Tabellen oft als grundlegende Datenstruktur eingesetzt werden. Dies ist zum Beispiel bei relationalen Datenbanken der Fall: Aus konzeptioneller oder externer Sicht betrachtet, bestehen Datensätze in relationalen Datenbanken aus Tabellen (streng genommen eigentlich aus Relationen). Auch in der Statistik werden Tabellen betrachtet: Üblich ist es, dass dann die Spalten Variablen und die Zeilen Beobachtungen (Observationen) entsprechen.

Wie wir sehen werden, besitzt R mit Data Frame eine mächtige Datenstruktur, die aus konzeptioneller Sicht einer Tabelle entspricht. Da eine Tabelle (und somit ein Data Frame) aus Zeilen und Spalten besteht, also zweidimensional ist, ist sie ähnlich zu einer Matrix. Aber es gibt einen bedeutenden Unterschied zwischen einer Matrix und einer Tabelle: In einer Tabelle können den einzelnen Spalten unterschiedliche Datentypen zugrunde liegen; eine Matrix hingegen ist homogen. Intern ist ein Data Frame im einfachsten Fall eine Liste mit der einschränkenden Bedingung, dass jedes Listenelement ein Vektor gleicher Länge ist. Insgesamt finden wir in R also folgende grundlegende Datenstrukturen vor:

Dimension | homogen | heterogen
----------|---------|-----------
`1` | `vector` (wie üblich meinen wir dabei einen atomaren Vektor), `factor` | `list`
`2` | `matrix` | `data frame`
`n` | `array` |

Wir unterscheiden hier zwischen Dimension (Wie bereits am Ende von Kapitel \@ref(homogenedatenstrukturen) erläutert, wird statt Dimension manchmal auch der Begriff „Anzahl der Achsen“ verwendet.) und Länge! Zum Beispiel betrachten wir einen (atomaren) Vektor als eine eindimensionale Datenstruktur, der allerdings eine beliebig große Länge haben kann.

### Data Frame erzeugen{#dataframeerzeugen}
...
Seit neueren R-Versionen wie z.B. R-4.2.1 ist in der Standardeinstellung von `data.frame()` `stringsAsFactors = FALSE` angegeben, wodurch verhindert wird, dass `character`-Vektoren in Faktoren umgewandelt werden (in früheren R-Versionen war dies z.T. noch nicht der Fall.), siehe folgendes Beispiel.
...
Hier wird ein Data Frame erzeugt, bestehend aus den Variablen `n`, `bezeichnung` und `mess1`.

```{r}
df_b <- data.frame(n, bezeichnung, mess1, stringsAsFactors = TRUE)
str(df_b)
```
...
Mithilfe von `apply()` können wir die Mittelwerte der gemessenen Zeiten ermitteln. Allgemein kann `apply()` angewandt werden, wenn die beteiligten Spalten denselben Datentyp haben. Die Mittelwerte für die 6 Observationen erhalten wir durch (Zur Erinnerung: bei `apply()` steht 1 für Zeilen und 2 für Spalten.):
...
Wie bei Matrizen können auch `cbind()` und `rbind()` benutzt werden, um neue Daten an jeder beliebigen Stelle einzufügen:
...
```{r}
l1 <- list(10:1, "Data Science",  c("Data Science", "FH SWF"), c(TRUE, TRUE, FALSE))
```
...
set.seed(1)
anzahl <-  100
student <- data.frame(MatNr = round(runif(anzahl, min= 100000, 999999)),
                       Punkte = round(rnorm(anzahl, mean = 65, sd = 20)),
                       Bestanden = FALSE,
                       Stdgang = sample(c("ET","MB"), anzahl, replace = TRUE))
```
...
In den Übungen zu Kapitel \@ref(homogenedatenstrukturen) haben wir bereits die Funktion `order()` kennengelernt.
...
d)	Erstellen Sie für den Notenspiegel ein entsprechendes Balkendiagramm (Benutzt wurden die Argumente: `main = "Notenspiegel"`,` xlab="Noten"`, `ylab="Anzahl"`): 
