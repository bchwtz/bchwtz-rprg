...
Parallel zu diesem Kapitel sollen Sie das Kapitel „5 Data transformation“  aus *„R for Data Sciences“* [@Wickhamny] durcharbeiten, das sich mit dem `dplyr`-Paket (dplyr steht für “**d**ata frame ap**plyer**”) auseinandersetzt, einer weiteren Kernkomponente der `tidyverse`-Welt.
...
In diesem Kapitel wollen wir mit der großen Datensammlung `flights` arbeiten, die im `nycflights13`-Paket [@nycflights13] enthalten ist; daher setzen wir zusätzlich voraus:
```{r, eval=FALSE}
...
Der Datensatz `flights` beschreibt alle Flüge, die 2013 von einem der drei New Yorker Flughäfen (JFK, LGA or EWR) gestartet sind (siehe: `?flights`). [@nycflights13] 
...
> "A concise, and fun, way to summarise the main differences is that tibbles are lazy and surly: they do less and complain more. You’ll see what that means as you work through this section." [@Wickham2019, Abschnitt 3.6]

Im Gegensatz zu einem Data Frame wird bei der Erzeugung eines Tibbles nicht automatisch ein `character`-Vektor in einen Faktor umgewandelt, was bei früheren R-Versionen der Fall war; siehe Abschnitt \@ref(dataframeerzeugen).
...
`flights` beschreibt also 336 776 Flüge mithilfe der folgenden 19 Variablen:
...
In `dplyr` gibt es fünf Schlüsselfunktionen (In [@Wickhamny] werden diese Schlüsselfunktionen auch als „Verben“ bezeichnet.), die jeweils ein Tibble (bzw. ein Data Frame) als erstes Element erwarten und als Resultat ein Tibble (bzw. ein Data Frame) zurückgeben.
...
**Hinweis:** (Dies ist der Hinweis aus den Übungen zu Kapitel \@ref(chap3):
...
`ends_with("xyz")` | sucht nach Namen, die mit "xyz" beginnen
...
`summarize()` wird sehr häufig im Zusammenspiel mit `group_by()` verwendet. Wir werden weiter unten hierauf detaillierter eingehen.


### Der Pipe-Operator `%>%`

Der Pipe-Operator `%>%` (In Visual Studio R fügt `Strg+Shift+m` den `%>%` ein) ist nicht Bestandteil von Base-R; er wird durch das `tidyverse`-Paket [@tidyverse] automatisch eingeführt und spielt in der `tidyverse`-Welt eine wichtige Rolle.
...
Gruppierungen sind in Verbindung mit `summarize()` sehr nützlich, werden oft auch bei `mutate()` and `filter()` eingesetzt. Im nächsten Beispiel wird zu `fluege_month` eine zusätzliche Variable hinzugefügt, die in jeder Zeile die entsprechende monatliche durchschnittliche Verspätung bei der Ankunft angibt (vgl. `summarize(fluege_month, delay = mean(arr_delay))`, siehe oben):
...
Die folgenden Aufgaben beziehen sich auf den Datensatz `flights` aus dem Paket `nycflights13` [@nycflights13].
...
c)	Gesucht sind alle Flüge in den Monaten September, Oktober, November und Dezember, die mit mehr als 150 Minuten Verspätung den Zielflughafen erreichten.
