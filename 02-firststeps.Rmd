...
```{r, echo=FALSE, fig.cap="Abb1", out.width="100%"}
...
```{r, echo=FALSE, fig.cap="Abb2", out.width="100%"}
...
```{r, echo=FALSE, fig.cap="Abb3", out.width="100%"}
...
```{r, echo=FALSE, fig.cap="Abb4", out.width="100%"}
...
Daneben gibt es hauptsächlich zu Übungszwecken unterschiedliche Datensätze, siehe ?datasets bzw. library(help = "datasets"). Wir werden kurz auf den Datensatz Nile eingehen, der die jährliche Durchflussmenge (in [$10^8 m^3$]) am Nil in Aswan (früher Assuan) zwischen 1871 und 1970 beschreibt.
...
```{r, echo=FALSE, fig.cap="Abb14", out.width="100%"}
...
a) Öffnen Sie das Skript „02_jw_psd_x.R“ und führen Sie es schrittweise aus.

```{r, echo=FALSE, eval=FALSE}
# TODO: Entspricht 02_jw_psd_x.R 02_jw_psd_b.R?
# Skript zum Downloaden hinterlegen!
```
...
df <- data.frame(col1 = c("sample(x, k)", "sample(x, k, TRUE)"),
                 col2 = c("Zufällige Auswahl von k Elementen aus x", "(ohne und mit Zurücklegen)"))
knitr::kable(t(df), booktabs = T, caption = "")%>%
...
(Random Walk, siehe auch: \url{https://de.wikipedia.org/wiki/Random_Walk})
...
(ref:rw) Eine Realisierung eines möglichen Wegs.

```{r, echo=FALSE, fig.cap="(ref:rw)", out.width="80%}
set.seed(1)
n <- 10e4
Schritte <- 1:n
