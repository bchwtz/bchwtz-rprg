...
Ein typisches Fenster in *RStudio* umfasst mehrere Bereiche. In Abbildung \@ref(fig:RStudio-Desktop-1) sehen Sie z. B. drei Bereiche: „Konsole“, „Umgebung“ und „Hilfe“. Im dort abgebildeten Konsolen-Bereich sehen wir zum Beispiel, welche R-Version und Plattform benutzt wird.
...
```{r RStudio-Desktop-1, echo=FALSE, fig.cap="RStudio Bereiche.", out.width="100%"}
knitr::include_graphics("gfx/abb1.png")
```

Im Bereich „Hilfe“ in Abbildung \@ref(fig:RStudio-Desktop-1) sehen Sie den Manual-Eintrag zum Befehl `.libPaths()`. Sie erhalten diesen Eintrag, indem Sie  `?.libPaths()` im Konsolenbereich hinter dem Prompt-Zeichen `'>'` eingeben. Wir werden später auf das Hilfesystem etwas ausführlicher eingehen.
...
Sie erhalten sofort das Ergebnis des eingegebenen Ausdrucks. Ist der eingegebene Ausdruck syntaktisch noch nicht abgeschlossen, dann wechselt in der Konsole das Prompt-Zeichen `'>'` zu `'+'`; mit einem Semikolon kann ein Abschluss erzwungen werden wie z.B.:
...
> - `%%` Modulo-Operator
> - Operatoren und Funktionen können direkt auf Vektoren angewandt werden.
...
Allgemein wird durch einen Ausdruck wie `n:m` (wobei `n` und `m` ganze Zahlen sind) ein Vektor erzeugt, dessen erstes Element `n` und dessen letztes Element `m` ist und aufeinanderfolgende Elemente sich entweder alle um 1 oder alle um -1 unterscheiden.
...
```{r, echo=FALSE, fig.cap="Die erzeugten globalen Variablen in RStudio im Bereich 'Umgebung' (Environment).", out.width="100%"}
...
Aus  diesem Beispiel kann man bereits erahnen, was die Funktion `seq()` bewirkt. Durch `help(seq)` oder kürzer durch `?seq` erhält man in RStudio den entsprechenden Manual-Eintrag (siehe Abbildung \@ref(fig:manual-eintrag-seq)). Weiterhin bezeichnet hier `pi` eine in R eingebaute Konstante, die der Kreiszahl entspricht (Verhältnis des Umfangs eines Kreises zu seinem Durchmesser).

```{r manual-eintrag-seq, echo=FALSE, fig.cap="(ref:manual-seq)", out.width="100%"}
knitr::include_graphics("gfx/abb3.png")
```

(ref:manual-seq) Manual-Eintrag der Funktion `seq()` angezeigt in RStudio.
...
```{r, eval=FALSE}
plot(x = x, y = sin(x), type = "l", col = "blue")
```

Durch `type = "l"` legen wir fest, dass die einzelnen Punkte durch Linien verbunden werden sollen, und durch `col = "blue"`, dass die Zeichenfarbe blau sein soll wie dies aus Abbildung \@ref(fig:first-plot-R) hervorgeht..

```{r first-plot-R, echo=FALSE, fig.cap="Plot in RStudio.", out.width="100%"}
...
Daneben gibt es hauptsächlich zu Übungszwecken unterschiedliche Datensätze; siehe `?datasets` bzw. `library(help = "datasets")` [@datasets]. Wir werden kurz auf den Datensatz `Nile` eingehen, der die jährliche Durchflussmenge $\left(\text{in }\left[10^8 m^3\right]\right)$ am Nil in Aswan (früher Assuan) zwischen 1871 und 1970 beschreibt.

```{r}
Nile
```

Oder in kompakter Darstellung:
...
Mit `summary()` erhalten wir diese beiden Werte zusammen mit vier anderen wichtigen statistischen Kennzahlen (Minimum, unteres und oberes Quartil und Median),  auf die wir an dieser Stelle nicht weiter eingehen wollen (siehe auch `?summary`):
...
```{r plot-Nile, fig.cap="(ref:plot-ts-Nile)"}
plot(Nile)
```

(ref:plot-ts-Nile) (Zeitreihen-)Diagramm des `Nile` Datensatzes [@datasets].

Ebenso sind ein Histogramm:

```{r hist-Nile, fig.cap="(ref:hist-ts-Nile)"}
hist(Nile)
```

(ref:hist-ts-Nile) Histogramm des `Nile` Datensatzes [@datasets]

und ein Boxplot schnell erstellt:

```{r boxplot-Nile, fig.cap="(ref:boxplot-ts-Nile)"}
boxplot(Nile)
```
...
```{r hist-10breaks, fig.cap="(ref:NV-breaks10)"}
hist(y)
```

(ref:NV-breaks10) Histogramm von 100.000 normalverteilten Zufallszahlen verteilt auf 100 Klassen.
...
```{r hist-100breaks, fig.cap="(ref:NV-breaks100)"}
hist(x, breaks = 100)
```

(ref:NV-breaks100) Histogramm von 100.000 normalverteilten Zufallszahlen verteilt auf 100 Klassen.

Wenn man Abbildung \@ref(fig:hist-100breaks) mit der „hochskalierten“ theoretischen Wahrscheinlichkeitsdichte der Normalverteilung in der nächsten Abbildung \@ref(fig:theoret-NV) vergleicht, dann sieht man, dass die so erzeugten Zufallszahlen diesen ersten groben Test bestanden haben. Für die Erzeugung der Abbildung \@ref(fig:theoret-NV) benutzen wir dabei in der Plot-Funktion die Funktion `dnorm()`, die die Werte der Wahrscheinlichkeitsdichte (density) der Normalverteilung zurückgibt: 

```{r theoret-NV, fig.cap="(ref:theoretische-NV)"}
z <- seq(-4, 4, by = 0.1)
plot(z, 100000*dnorm(z), type="l", col="blue")
```

(ref:theoretische-NV) Wahrscheinlichkeitsdichte der Normalverteilung $N(0,1)$ multipliziert mit $10^5$.
...
Auch wollen wir wieder die Histogramme des kleinen und des großen Datensatzes erzeugen:

```{r hist-equ-small, fig.cap="(ref:hist-gleichv-small)"}
hist(y)
```

(ref:hist-gleichv-small) Histogramm von 1.000 gleichverteilten Zufallszahlen im Intervall [0,1] verteilt auf 10 Klassen.

```{r hist-equ, fig.cap="(ref:hist-gleichv)"}
hist(x, breaks = 100)
```

(ref:hist-gleichv) Histogramm von 100.000 gleichverteilten Zufallszahlen im Intervall [0,1] verteilt auf 100 Klassen.
...
## Einführung in die Entwicklungsumgebung von R (fortgeschritten)

**Lernziel:**

> - Sie sollen in der Lage sein, Hilfsmittel in der Entwicklungsumgebung von R zu nutzen, um R Skripte auf Fehler zu untersuchen und diese zu identifizieren.
...
2. Wir vermuten, dass der Fehler innerhalb der `for`-Schleife liegt. Folglich würde es uns helfen, wenn wir die Schleife schrittweise durchlaufen und überprüfen würden. Dies erreichen wir, indem wir einen *Breakpoint* in unserer Schleife z.B. mittels `browser()` wie folgt setzen [@McPherson2022]:
...
Für einige mag das Problem bei diesem Code vielleicht sofort offensichtlich gewesen sein. Liegt jedoch ein deutlich umfangreicherer Code vor, so ist dieses Problem nicht sofort offensichtlich und *Debugging* ist nicht mehr wegzudenken.

Soll der *Debugging*-Modus beendet werden, kann entweder auf *Stop* (siehe Abbildung \@ref(fig:abb-debug2)) geklickt oder die `Esc` genutzt werden.
...
a) Speichern Sie diesen Code in Form eines Skripts und führen Sie es anschließend schrittweise aus. Ergänzen Sie Kommentare.
