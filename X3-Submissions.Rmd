...
devtools::install_github("StrategicProjects/webdav")
library(webdav)
library(tidyverse)
install.packages("curl", force=TRUE)
```
...
```{r, echo=FALSE, error=TRUE}
f <- webdav_list_files(
  base_url = "https://fh-swf.sciebo.de/public.php/webdav/", 
  folder_path = "EX2", 
  username = Sys.getenv("WEBDAV_USER"), 
  password = Sys.getenv("WEBDAV_PW"), 
  depth = 1, 
  verbose = FALSE
)
...
f %>% 
  ...
  select( -full_path, -http_status, -content_type, -content_length, -contents) %>%
  # Sort the tibble
  arrange(desc(OK), id, match, iteration) %>% 
  # Rearrange columns
  relocate(OK, id, iteration, last_modified, file) 
#%>% 
  # Output table
 # kbl() %>%
  #kable_styling()
