...
```{r}
tbl %>% kable(format = "pipe")
```


```{r, error=TRUE}
tbl %>% kbl() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10)
```

```{r, error=TRUE}

tbl %>% kbl() %>% kable_styling(bootstrap_options = c("hover"), font_size = 10) %>% column_spec(2, color = ifelse(tbl$OK, "green", "red"))
```
